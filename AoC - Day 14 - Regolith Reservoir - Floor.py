...
def sand_fall(rocksCoords: set, sandsCoords: set, sandSourceCoords: tuple) -> set:
    ...
    maxY += 2       # floor is 2 tiles below the maximum depth

    while sandCoords[1] < maxY:
        if sandCoords[1] == maxY - 1:
            return sandsCoords.union({sandCoords})  # use an if statement rather than generating a rock on depth floor
